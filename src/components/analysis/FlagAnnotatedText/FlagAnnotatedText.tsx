import { memo } from "react";
import { motion } from "framer-motion";
import type { AnnotatedSpan } from "../../../hooks/useAnnotatedSpans.ts";
import type { Flag } from "../../../types/analysis.types.ts";
import Popover from "../../ui/Popover/Popover.tsx";
import Card from "../../ui/Card/Card.tsx";

interface FlagAnnotatedTextProps {
  spans: AnnotatedSpan[];
  onFlagClick?: (flag: Flag) => void;
}

const riskBgClasses = "bg-red-100 dark:bg-red-900/30";
const riskTextClasses = "text-red-800 dark:text-red-300";
const opportunityBgClasses = "bg-green-100 dark:bg-green-900/30";
const opportunityTextClasses = "text-green-800 dark:text-green-300";

function AnimatedHighlightSpan({
  children,
  variant,
  onClick,
  onKeyDown,
  delay = 0,
}: {
  children: string;
  variant: "risk" | "opportunity";
  onClick: () => void;
  onKeyDown: (e: React.KeyboardEvent) => void;
  delay?: number;
}) {
  const bgClasses =
    variant === "risk" ? riskBgClasses : opportunityBgClasses;
  const textClasses =
    variant === "risk" ? riskTextClasses : opportunityTextClasses;

  // Shorter span = slower; longer span = faster
  const duration = Math.max(1.35, Math.min(1.4, 1.5 - children.length * 0.035));

  return (
    <span
      className="relative inline overflow-hidden rounded px-0.5 cursor-pointer hover:underline"
      role="button"
      tabIndex={0}
      onClick={onClick}
      onKeyDown={onKeyDown}
    >
      <motion.span
        className={`absolute inset-y-0 left-0 rounded ${bgClasses}`}
        initial={{ width: 0 }}
        animate={{ width: "100%" }}
        transition={{
          duration,
          ease: [0.22, 0.61, 0.36, 1],
          delay,
        }}
      />
      <span className={`relative z-10 ${textClasses}`}>{children}</span>
    </span>
  );
}

const QUOTE_PATH =
  "M150.299,26.634v58.25c0,7.9-6.404,14.301-14.304,14.301c-28.186,0-43.518,28.909-45.643,85.966h45.643 c7.9,0,14.304,6.407,14.304,14.304v122.992c0,7.896-6.404,14.298-14.304,14.298H14.301C6.398,336.745,0,330.338,0,322.447V199.455 c0-27.352,2.754-52.452,8.183-74.611c5.568-22.721,14.115-42.587,25.396-59.048c11.608-16.917,26.128-30.192,43.16-39.44 C93.886,17.052,113.826,12.333,136,12.333C143.895,12.333,150.299,18.734,150.299,26.634z M334.773,99.186 c7.896,0,14.305-6.407,14.305-14.301v-58.25c0-7.9-6.408-14.301-14.305-14.301c-22.165,0-42.108,4.72-59.249,14.023 c-17.035,9.248-31.563,22.523-43.173,39.44c-11.277,16.461-19.824,36.328-25.393,59.054c-5.426,22.166-8.18,47.266-8.18,74.605 v122.992c0,7.896,6.406,14.298,14.304,14.298h121.69c7.896,0,14.299-6.407,14.299-14.298V199.455 c0-7.896-6.402-14.304-14.299-14.304h-44.992C291.873,128.095,306.981,99.186,334.773,99.186z";

function AnimatedQuoteIcon({
  className = "",
  mirror = false,
}: {
  className?: string;
  mirror?: boolean;
}) {
  return (
    <motion.svg
      width={60}
      viewBox="0 0 349.078 349.078"
      fill="none"
      stroke="currentColor"
      strokeWidth={8}
      strokeLinecap="round"
      strokeLinejoin="round"
      className={`text-gray-800 dark:text-gray-200 ${mirror ? "-scale-x-100" : ""} ${className}`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.3 }}
    >
      <motion.path
        d={QUOTE_PATH}
        initial={{ pathLength: 0.000 }}
        animate={{ pathLength: 1 }}
        transition={{
          duration: 5,
          ease: [0.22, 0.61, 0.36, 1],
        }}
      />
    </motion.svg>
  );
}

function FlagDetails({ flag }: { flag: Flag }) {
  return (
    <div className="space-y-2">
      <h4 className="font-semibold text-lg text-gray-900 dark:text-white">
        {flag.title}
      </h4>
      <p className="text-md text-gray-600 dark:text-gray-300">
        {flag.description}
      </p>
      <p className="text-sm text-gray-500 dark:text-gray-400">
        <span className="font-semibold text-gray-700 dark:text-gray-300">Suggestion:</span> {flag.suggestion}
      </p>
    </div>
  );
}

function FlagAnnotatedText({ spans, onFlagClick }: FlagAnnotatedTextProps) {
  return (
    <Card title="Annotated Ad Text" animationDelay={0.2}>
      <div className="flex items-center justify-center gap-5">
        <AnimatedQuoteIcon className="shrink-0" />
        <div className="inline text-base leading-relaxed text-gray-800 dark:text-gray-200">
          {spans.map((s, i) => {
            if (s.flag) {
              return (
                <Popover
                  key={i}
                  trigger={
                    <AnimatedHighlightSpan
                      variant={s.flag.type}
                      delay={i * 0.15}
                      onClick={() => onFlagClick?.(s.flag!)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter" || e.key === " ") {
                          e.preventDefault();
                          onFlagClick?.(s.flag!);
                        }
                      }}
                    >
                      {s.text}
                    </AnimatedHighlightSpan>
                  }
                  content={<FlagDetails flag={s.flag} />}
                  className="inline"
                />
              );
            }
            return <span key={i}>{s.text}</span>;
          })}
        </div>
      </div>
    </Card>
  );
}

export default memo(FlagAnnotatedText);
